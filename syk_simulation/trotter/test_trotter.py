...
from syk_simulation.trotter import first_order_trotter, second_order_trotter, trotter_evolution
...
def test_first_order_simple():
    ...
    hamiltonian = PauliSum([0.5, PauliMask(0b11, 0b00)], [0.3, PauliMask(0b00, 0b11)])
...
def test_convergence_with_trotter_steps():
    ...
    for i in range(len(fidelities) - 1):
        assert fidelities[i + 1] >= fidelities[i] - 1e-10, "Fidelity should improve with more steps"


def test_trotter_evolution_interface():
    num_qubits = 2

    hamiltonian = PauliSum([0.5, PauliMask(0b01, 0b00)], [0.3, PauliMask(0b00, 0b11)])
...
def test_compare_with_workbench():
    ...
    ham = PauliSum([1.0, PauliMask(1, 0)], [0.7, PauliMask(0, 3)])  # X0  # Z0Z1
